...
export function initShopPage() {
  ...
  function render() {
    ...
    // Jika belum ada produk, retry setelah 300ms (tunggu seed selesai)
    if (all.length === 0) {
  ...
  // Tampilkan skeleton dulu, lalu render
  renderSkeletons(grid, 8);
  render();
}
...
export function initFeaturedSection() {
  ...
  // Tunggu sampai produk tersedia
  const products = getAllProducts();
  if (products.length === 0) {
    grid.innerHTML = '<div class="col-span-full text-center py-8 text-gray-400">Memuat produk...</div>';
    setTimeout(initFeaturedSection, 300);
    return;
  }

  grid.innerHTML = '';
  getFeaturedProducts(12).forEach(p => grid.appendChild(renderProductCard(p)));
}
