...
async function deleteProduct(id) {
  ...
  try {
    ...
    Toast.show('Gagal menghapus produk', 'error');
  }
}
...
async function saveProduct(editId) {
  ...
  try {
    const { API } = await import('./api.js');
    const productData = { name, category, price, stock, image, description, tags, featured, rating: 4.0 };
    ...
    console.error(error);
    Toast.show('Terjadi kesalahan saat menyimpan produk.', 'error');
  }
}
